...
public class MxMediaManager implements IMediaPlayer.OnPreparedListener, IMediaPlayer.OnCompletionListener,
    ...
    public int mainvideowidth = 0;
    public int mainvideoheight = 0;
    ...
    public void prepare(final String url, final Map<String, String> mapHeapData, boolean loop) {
        if (!TextUtils.isEmpty(url)) {
            ...
            if (mMediaHandler.getLooper().getThread().isAlive()) {
    ...
    public void onVideoSizeChanged(IMediaPlayer mp, int width, int height, int sar_num, int sar_den) {
        ...
        Log.d(TAG, "onVideoSizeChanged: " + width + "" + height + "???" + mCurVideoHeight + ">" + mCurVideoWidth);
        if (firsttimechange) {
            firsttimechange = false;
            mainvideowidth = mCurVideoWidth;
            mainvideoheight = mCurVideoHeight;
        }
        mainThreadHandler.post(new Runnable() {
            @Override
            public void run() {
            ...
            switch (msg.what) {
                case HANDLER_PREPARE:
                    try {
                        ...
                        // mMediaPlayer.setc
                        //   Log.d(TAG, "handleMessage: " + mMediaPlayer.getAudioCachedDuration());
...
//                        IjkTrackInfo[] vals = mMediaPlayer.getTrackInfo();
//                        Log.e(TAG, "MxMediaManager: "+vals.length );
                    } catch (Exception e) {
                        e.printStackTrace();
